...
import { authors, editions, books, tags, bookTags } from '../drizzle/schema';
...
async function importData() {
  ...
  const uniqueEditions = new Map<string, boolean>();
  ...
  const uniqueAuthors = new Map<string, boolean>();
  for (const row of excelData) {
  ...
  for (const row of excelData) {
    ...
    const [book] = await db.insert(books).values({
      ...
      editionId: editionId || defaultEdition.id,
      quantity,
      price,
      isbn,
      description,
